...
PacketCaptureDialog::PacketCaptureDialog(QWidget* parent) : QDialog(parent) {
    setWindowTitle(tr("Packet Capture"));
    resize(680, 460);
    ...
    m_filter->setPlaceholderText(tr("BPF filter, e.g. \"tcp port 22\" (optional)"));
    m_btn = new QPushButton(tr("Start"), this);
    ...
    m_table->setHorizontalHeaderLabels({QStringLiteral("#"), tr("Proto"),
                                        tr("Info")});
    ...
    if (!tools::PacketCapture::available())
        m_status->setText(tr("Built without libpcap — capture unavailable"));
    ...
    connect(&m_capture, &tools::PacketCapture::failed, this,
            [this](const QString& m) { m_status->setText(m); m_btn->setText(tr("Start")); });
}

void PacketCaptureDialog::toggle() {
    if (m_capture.isRunning()) {
        m_capture.stop();
        m_btn->setText(tr("Start"));
        m_status->setText(tr("Stopped (%1 packets)").arg(m_count));
        return;
    }
    m_count = 0;
    m_table->setRowCount(0);
    if (m_capture.start(m_iface->currentText(), m_filter->text())) {
        m_btn->setText(tr("Stop"));
        m_status->setText(tr("Capturing on %1…").arg(m_iface->currentText()));
    }
}
...
void PacketCaptureDialog::onPacket(const macxterm::tools::PacketSummary& s) {
    ...
    m_table->setItem(row, 2, new QTableWidgetItem(
        s.info.isEmpty() ? tr("%1 bytes").arg(s.length) : s.info));
