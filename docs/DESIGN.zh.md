...
| Windows 相依套件不在 vcpkg 中 | 內載（vendor）libvterm（neovim fork）+ libssh（GitHub 鏡像），於 configure 階段建置 | libvterm 沒有 vcpkg port；libssh 的 vcpkg port 會從 libssh.org 下載，而該站並非總是可連。兩者都能用 vcpkg toolchain 乾淨建置（見 `scripts/win/build-*.ps1`）。 |
| Windows Unix 使用者空間 | 以**獨立行程方式呼叫內附的 BusyBox**（不連結）；透過 `scripts/win/fetch-userland.ps1` 取得，絕不 commit 進倉庫 | 在保有「不*連結* GPL 程式碼」規則的前提下，提供 MobaXterm 風格的本地 Unix 終端機（BusyBox 為 GPL）——與 Mosh 相同的行程隔離決策。`core::CygPath` 的 `/drives`↔`C:\` 對映是 MIT 授權且在倉庫內。 |
| Windows SSH 家族傳輸 | 在既有的跨平台 `libssh2`/`libssh` 之下加一層薄薄的 `platform/Net` shim（WSAStartup / Winsock）；Unix 路徑則展開為完全相同的系統呼叫 | 讓 SSH/SFTP/通道/X11/伺服器在 Windows 上運作，無需分叉協定程式碼，也不擾動 macOS/Linux 的行為。 |
...
- **隨附打包** Windows X server——Windows 上的 X11 轉發會自動偵測並啟動
  使用者已安裝的 VcXsrv（免設定），但在安裝程式中*隨附發行* VcXsrv 執行檔
  仍屬延後（第三方產物）。

**Windows 對等——現已納入範疇（原本「範疇之外」）**
那些讓 MobaXterm 成為 Windows 產品的 Windows 專屬功能，正朝 100% 對等推進，
追蹤於 **[docs/WINDOWS_SPRINT.md](WINDOWS_SPRINT.md)**：
- **本地 ConPTY shell** — ✅ 現已可運作：`_WIN32` 的 `Pty` 會執行一條背景
  讀取執行緒，把輸出灌入 `readyRead()`，並在子行程結束時發出 `finished()`；
  `LocalShellConnection` 會退回使用 `%ComSpec%` / `%USERPROFILE%`。
- **Windows 上的 SSH 家族** — ✅ 現已透過 Winsock 傳輸運作。一個新的
  `platform/Net` shim（WSAStartup、connectTcp、closeSocket、setNonBlocking、
  pollReadable、dupSocket、loopback socketPair）支撐 `libssh2`，因此 SSH、SFTP、
  local/remote/dynamic-SOCKS 通道與 X11 轉發全都能在 Windows 上運行。該 shim
  的 Unix 路徑與先前內嵌的系統呼叫逐位元組相同，因此 macOS/Linux 不受影響。
- **本次 session 落地的其他 Windows 對等項目：** WSL session（ConPTY +
  `shellargs`）、PuTTY/WinSCP session 匯入、DPAPI 帳號綁定保管庫
  （`CryptProtectData`）、一個把 Win32 `_wstat64`→fattr3 對映起來、
  解除 NFS 伺服器樁（stub）的實作、`cygpath`/`/drives` 路徑對映、shell 整合
  （協定 + 檔案關聯）、免設定 VcXsrv，以及**內嵌 SSH/SFTP 伺服器**（libssh
  由其 GitHub 鏡像 vendored；`SshServer` 移植成 ConPTY shell + Qt-based SFTP，
  已實機驗證）。剩下的項目只需外部產物（隨附的 busybox/VcXsrv 執行檔、
  程式碼簽章憑證）——見 WINDOWS_SPRINT.md。
- WSL session、隨附的 Cygwin 風格本地 Unix 使用者空間（`/drives`·`cygpath`）、
  PuTTY 登錄檔／WinSCP 匯入、DPAPI 保管庫綁定、VcXsrv 免設定 X11，以及
  Windows 的 shell／協定處理常式——規劃於 Sprints W3–W7。
- **MobApt** 套件管理員仍延後，待 W4 使用者空間決策定案。
...
| ▢ 剩餘的協定深度 | XDMCP 顯示轉導、VNC Tight；隨附 X server。 |
| ◐ Windows 100% 對等 | 完整 app 在 Windows 上可建置／連結／啟動 ✅（MSVC+Qt+vcpkg、vendored libvterm）；ConPTY shell ✅。剩下：Winsock SSH 傳輸（SSH 為樁）、WSL、Cygwin 風格使用者空間、PuTTY/WinSCP 匯入、DPAPI、VcXsrv——見 [WINDOWS_SPRINT.md](WINDOWS_SPRINT.md)。 |
